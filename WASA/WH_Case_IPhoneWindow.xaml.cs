...
{
    /// <summary>
    /// Логика взаимодействия для WH_Case_IPhoneWindow.xaml
    /// </summary>
    public partial class WH_Case_IPhoneWindow : Window
    {
        private static GlobalData globalData = new GlobalData();
        private static FileIOService fileIOService = new FileIOService();
        private ObservableCollection<WareHouseModel> _warehouse_data = fileIOService.LoadObservableData(globalData.GetDataPath("Case_IPhone_Data.json"));
        private BindingList<WareHouseModel> _tempdata = fileIOService.LoadTempData();
        
        public WH_Case_IPhoneWindow()
        {
            InitializeComponent();
        ...
        {
            WH_Case_IPhoneWindowEdit wH_Case_IPhoneWindow = new WH_Case_IPhoneWindowEdit();
