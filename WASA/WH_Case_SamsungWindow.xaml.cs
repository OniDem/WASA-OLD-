...
{
    /// <summary>
    /// Логика взаимодействия для WH_CaseWindow.xaml
    /// </summary>
    public partial class WH_Case_SamsungWindow : Window
    {
        private static GlobalData globalData = new GlobalData();
        private static FileIOService fileIOService = new FileIOService();
        private ObservableCollection<WareHouseModel> _warehouse_data = fileIOService.LoadObservableData(globalData.GetDataPath("Case_Samsung_Data.json"));
        private BindingList<WareHouseModel> _tempdata = fileIOService.LoadTempData();
        
        public WH_Case_SamsungWindow()
        {
            InitializeComponent();
        ...
        {
            WH_Case_SamsungWindowEdit wH_Case_SamsungWindowEdit = new WH_Case_SamsungWindowEdit();
            wH_Case_SamsungWindowEdit.Show();
            Close();
        }
