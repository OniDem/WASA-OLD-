...
{
    /// <summary>
    /// Логика взаимодействия для WH_CarHolderWindow.xaml
    /// </summary>
    public partial class WH_CarHolderWindow : Window
    {
        private static GlobalData globalData = new GlobalData();
        private static FileIOService fileIOService = new FileIOService();
        private ObservableCollection<WareHouseModel> _warehouse_data = fileIOService.LoadObservableData(globalData.GetDataPath("CarHolder_Data.json"));
        private BindingList<WareHouseModel> _tempdata = fileIOService.LoadTempData();
        
        public WH_CarHolderWindow()
        {
            InitializeComponent();
        ...
        {
            WH_CarHolderWindowEdit wH_CarHolderWindow = new WH_CarHolderWindowEdit();
