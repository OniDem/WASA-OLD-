...
{
    /// <summary>
    /// Логика взаимодействия для WH_ChargeWindow.xaml
    /// </summary>
    public partial class WH_ChargeWindow : Window
    {
        private static GlobalData globalData = new GlobalData();
        private static FileIOService fileIOService = new FileIOService();
        private ObservableCollection<WareHouseModel> _warehouse_data = fileIOService.LoadObservableData(globalData.GetDataPath("Charge_Data.json"));
        private BindingList<WareHouseModel> _tempdata = fileIOService.LoadTempData();
       

        public WH_ChargeWindow()
        {
            InitializeComponent();
        ...
        {
            WH_ChargeWindowEdit wH_ChargeWindowEdit = new WH_ChargeWindowEdit();
            wH_ChargeWindowEdit.Show();
            Close();
        }
