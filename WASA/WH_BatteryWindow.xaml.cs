...
{
    /// <summary>
    /// Логика взаимодействия для WH_BatteryWindow.xaml
    /// </summary>
    public partial class WH_BatteryWindow : Window
    {
        private static GlobalData globalData = new GlobalData();
        private static FileIOService fileIOService = new FileIOService();
        private ObservableCollection<WareHouseModel> _warehouse_data = fileIOService.LoadObservableData(globalData.GetDataPath("Battery_Data.json"));
        private BindingList<WareHouseModel> _tempdata = fileIOService.LoadTempData();
        
        public WH_BatteryWindow()
        {
            InitializeComponent();
        ...
        {
            WH_BatteryWindowEdit wH_BatteryWindowEdit = new WH_BatteryWindowEdit();
            wH_BatteryWindowEdit.Show();
            Close();
        }
