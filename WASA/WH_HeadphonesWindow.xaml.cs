...
{
    /// <summary>
    /// Логика взаимодействия для WH_HeadphonesWindow.xaml
    /// </summary>
    public partial class WH_HeadphonesWindow : Window
    {
        private static GlobalData globalData = new GlobalData();
        private static FileIOService fileIOService = new FileIOService();
        private ObservableCollection<WareHouseModel> _warehouse_data = fileIOService.LoadObservableData(globalData.GetDataPath("Headphones_Data.json"));
        private BindingList<WareHouseModel> _tempdata = fileIOService.LoadTempData();
        

        public WH_HeadphonesWindow()
        {
            InitializeComponent();
        ...
        {
            WH_HeadphonesWindowEdit wH_HeadphonesWindowEdit = new WH_HeadphonesWindowEdit();
            wH_HeadphonesWindowEdit.Show();
            Close();
        }
