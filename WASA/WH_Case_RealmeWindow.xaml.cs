...
{
    /// <summary>
    /// Логика взаимодействия для WH_Case_RealmeWindow.xaml
    /// </summary>
    public partial class WH_Case_RealmeWindow : Window
    {
        private static GlobalData globalData = new GlobalData();
        private static FileIOService fileIOService = new FileIOService();
        private ObservableCollection<WareHouseModel> _warehouse_data = fileIOService.LoadObservableData(globalData.GetDataPath("Case_Realme_Data.json"));
        private BindingList<WareHouseModel> _tempdata = fileIOService.LoadTempData();

        public WH_Case_RealmeWindow()
        {
            InitializeComponent();
        ...
        {
            WH_Case_RealmeWindowEdit wH_Case_RealmeWindowEdit = new WH_Case_RealmeWindowEdit();
            wH_Case_RealmeWindowEdit.Show();
            Close();
        }
